...
const ExclusiveForYou = ({ allProductsData }) => {
        ...
        {loading
          ? (
            <div className='grid md:grid-cols-4 gap-4 animate-fade-right'>
              {JSON.parse(allProductsData).slice(0, 4).map((offer, index) => (
                <div
                  key={offer.product} className=''
                >
                  <Card className='p-0 rounded-md hover:scale-[102%] 3xl:h-[550px] md:h-[500px] h-[300px] relative' href={`/product/${offer.record_id}`} as={Link}>
                    <CardBody className=' flex flex-col justify-between h-52 p-0 overflow-hidden z-0'>
                      <div className='flex flex-col justify-between py-1 px-2 gap-4 relative'>
                        <h4 className=' text-large text-white italic whitespace-nowrap truncate uppercase text-center'>{offer.product}
                          <div className='bg-black absolute left-0 -top-1 w-full h-[50%] bg-opacity-70 -z-10' />
                        </h4>
                        <Chip
                          className={`${offer.category === 'waste' ? 'from-indigo-700 via-purple-700 to-transparent to-90%' : 'from-lime-700 via-green-700 to-transparent to-90%'} bg-transparent bg-gradient-to-r font-black pr-10 italic rounded-md uppercase text-small text-white`}
                        >
                          {offer.category}
                        </Chip>
                      </div>
