...
const LineGraph = ({ data, monthsTicks = 4, xValue = 'time', yValue = 'value', graphWidth = 550, graphHeight = 400, weekNumberTicks = false, xAxisTitle = '', yAxisTitle = '', verticalTooltip = true, lineLimit = 4, showPositiveSign = false, decimalPlaces = 2 }) => {
  ...
  useEffect(() => {
    ...
    if (data[0]?.data != undefined) {
      ...
      const newTooltip = select('body').append('div')
        .attr('class', 'tooltip')
        .style('opacity', 0)
      ...
      if (verticalTooltip == true && data.length < lineLimit) {
        ...
        overlay.on('mousemove', function (event) {
          ...
          const tooltipX = `${closestDateString}<br/>`
          const tooltipY = filteredTooltipData.map(item => item.name != '' ? `${item.name}: ${showPositiveSign ? parseFloat(item.value) > 0 ? '+' : '' : ''}${Number(item.value).toFixed(decimalPlaces)}` : `${Number(item.value).toFixed(decimalPlaces)}`).join('<br/>')
        ...
        if (series.dottedLine) {
          ...
          if (data.length < lineLimit) {
            ...
            invisiblePath.on('mousemove', function (event) {
              ...
              const tooltipX = `${closestDateString}<br/>`
              const tooltipY = tooltipData.map(item => `${item.name}: ${showPositiveSign ? parseFloat(item.value) > 0 ? '+' : '' : ''}${(item.value)}`).join('<br/>')
              const tooltipContent = tooltipX + tooltipY
