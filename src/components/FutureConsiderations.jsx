        ...
        {itemsToDisplay &&
          itemsToDisplay.map((snapshot, index) => (
            <div key={index} className={`${index === 0 && itemsToDisplay.length === 3 ? ' row-span-2' : ''}`}>
              <div className='h-full border relative hover:scale-[103%] transition-transform duration-300 shadow-lg rounded-lg w-full cursor-pointer flex gap-2 overflow-hidden' onClick={() => setSnapshotPopup(snapshot)}>
                <Image
