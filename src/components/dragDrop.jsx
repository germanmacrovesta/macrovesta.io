...
const DragDrop = (props) => {
  ...
  const handleCreateReport = async () => {
    ...
    // API endpoint where we send form data.
    const endpoint = '/api/generate-custom-report'
          ...
          <div className='relative flex flex-col w-full items-center mb-36'>
            <div className='col-span-2 pt-4  flex flex-col text-lg text-black text-center'>
              <div className='w-[140mm] text-sm'>
                Below, you can choose from our premade <b>templates</b> or make your own and save it for future use.
              </div>
            ...
            {!generatingReport && (
              <div className="col-span-2 mt-8 font-semibold text-lg bg-deep_blue text-white cursor-pointer rounded-xl shadow-lg px-2 py-1 text-center" onClick={handleCreateReport}>Create Report

              </div>
            )}
            {generatingReport && !generatedReport && (
              <>
                <div className="col-span-2 mt-8 font-semibold text-lg text-black  px-2 py-1 text-center">Generating Report Now
            ...
            {generatedReport && (
              <>
                <div className="col-span-2 mt-8 font-semibold text-lg text-black px-2 py-1 text-center">Report Generated</div>
            ...
            <Droppable droppableId={`${0}`} direction="vertical">
              {(provided, snapshot) => (
                ...
                >
                  ...
                  <div className='bg-gradient-to-tr from-gray-300 to-gray-500 z-50 h-screen overflow-y-auto overflow-x-clip'>

                    <div className="z-50 col-span-2 h-fit py-8 font-semibold text-lg text-white">
                        ...
                        <Draggable key={`${item.id}`} draggableId={`${item.id}`} index={index} id={`${item.id}`}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              {...provided.draggableProps}
                              {...provided.dragHandleProps}
                              className={` ${`col-span-${item.width}`} w-full h-fit   rounded-lg cursor-move select-none relative box-border`}
                            >
                              <div >
